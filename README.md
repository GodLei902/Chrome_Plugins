...
填写完成后点击“开始”。点击开始前会再次保存并校验配置；如果 URL 不完整，或任意一组最小值大于最大值，页面会提示错误。
...
2. 保持“预览模式”开启，配置会自动保存。
3. 打开完全匹配的目标帖子页面。
4. 等待 Instagram 页面加载完成，在右下角找到“社交评论清理器”面板。
5. 点击配置页底部的“开始”，或点击 Instagram 页面右下角面板中的“开始”。
...
1. 返回设置页关闭“预览模式”，配置会自动保存。
2. 回到目标帖子，在配置页点击“开始”，或点击页面右下角面板中的“开始”。
3. 扫描完成后会自动处理符合规则的评论。
