...
正式运行只删除命中的子级回复，一级评论永远不会删除。每次删除成功后会重新扫描并重建候选，达到“每轮最多删除”后按休息时间边界随机暂停，时间到后自动继续；也可以点击“暂停”暂存进度，点击“继续”恢复，点击“停止”终止本轮任务。扫描时会点击当前评论区已经显示的“查看回复/加载更多评论或回复”入口，并等待新的评论 ID 出现后再筛选；不会主动滚动页面寻找不可见的分页入口。当前没有可见加载入口且没有候选时，任务完成。
