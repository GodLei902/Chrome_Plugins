...
面板会显示当前状态，以及“扫描、删除、跳过、已加载回复、滚动轮次”统计。预览模式不会执行删除；白名单回复、帖子作者回复和未命中关键词的回复会被跳过。

### 3. 执行正式删除

确认预览结果无误后，在配置页点击“开始”。扩展会自动跳转到目标页面，页面加载完成后立即扫描并自动处理符合规则的评论；无需再点击页面面板中的“开始”或删除确认。

正式运行只删除命中的子级回复，一级评论永远不会删除。每次删除成功后会重新扫描并重建候选，达到“每轮最多删除”后按休息时间边界随机暂停，时间到后自动继续；也可以点击“暂停”暂存进度，点击“继续”恢复，点击“停止”终止本轮任务。连续三轮没有新的可加载回复时自动完成，不刷新页面。
...
- 一级评论永远不会删除；只处理通过正常展开和滚动加载到的子级回复，避免未知级联回复被连带删除。
- 删除后会持续滚动当前帖子评论区域；连续三轮没有新回复、展开入口或候选后才完成，不跨帖子抓取。
...
确认评论已经加载、关键词没有多余空格，并检查关键词是否确实出现在回复文本中。扩展会自动滚动并展开当前帖子可正常加载的评论区域；若 Instagram 未继续返回新内容，达到空轮阈值后才完成。
