...
for (const file of ['src/shared/delay-generator.js', 'src/shared/scheduled-rest.js', 'src/shared/task-session.js', 'src/shared/action-pace-config.js', 'src/shared/backoff.js', 'src/shared/action-pace-controller.js', 'src/shared/rate-limiter.js', 'src/shared/comment-surface-stability.js', 'src/platform/instagram/pagination-surface.js', 'src/platform/instagram/pagination-controls.js', 'src/platform/instagram/pagination-loader.js']) vm.runInNewContext(fs.readFileSync(file, 'utf8'), context);
test('延迟始终在配置范围内', () => { const config = { distribution: 'log-normal', meanSeconds: 18, minSeconds: 12, maxSeconds: 30, variability: 'high' }; for (let i = 0; i < 100; i += 1) { const value = context.DelayGenerator.generateDelayMs(config); assert.ok(value >= 12000 && value <= 30000); } });
test('状态机在连续上限后休息并重置', () => { const pace = new context.SocialCommentActionPaceController({ maxConsecutive: 2, backoff: { maxFailures: 3 } }); pace.begin(); assert.equal(pace.success(), 'NORMAL'); pace.begin(); assert.equal(pace.success(), 'REST'); pace.restComplete(); assert.equal(pace.consecutive, 0); assert.equal(pace.state, 'NORMAL'); });
test('统一动作协调器依次申请额度、等待并执行，取消后不点击', async () => {
  const pace = new context.SocialCommentActionPaceController({ maxConsecutive: 2, backoff: { maxFailures: 3 } });
  const calls = [];
...
test('限频窗口不超额且返回等待时间', () => { const limiter = new context.SocialCommentRateLimiter(); assert.equal(limiter.acquire({ perMinute: 2, perHour: 3 }, 1000).ok, true); assert.equal(limiter.acquire({ perMinute: 2, perHour: 3 }, 1001).ok, true); const blocked = limiter.acquire({ perMinute: 2, perHour: 3 }, 1002); assert.equal(blocked.ok, false); assert.ok(blocked.retryAfterMs > 0); });
test('旧设置迁移到节奏配置', () => { const settings = context.SocialCommentPaceConfig.normalizeSettings({ deleteDelayMin: 10, deleteDelayMax: 20, cooldownMin: 40, cooldownMax: 80, batchLimit: 4 }); assert.equal(settings.pace.operation.meanSeconds, 15); assert.equal(settings.pace.rest.meanSeconds, 60); assert.equal(settings.pace.maxConsecutive, 4); });
test('节奏默认值使用新的批次与休息边界', () => { const settings = context.SocialCommentPaceConfig.normalizeSettings({}); assert.equal(settings.pace.maxConsecutive, 20); assert.equal(settings.pace.rest.meanSeconds, 60); assert.equal(settings.pace.rest.minSeconds, 45); assert.equal(settings.pace.rest.maxSeconds, 90); assert.equal(settings.sessionLimit, 100); });
test('删除前等待默认值使用 5 至 25 秒范围', () => { const settings = context.SocialCommentPaceConfig.normalizeSettings({}); assert.equal(settings.pace.deleteDialogDelay.meanSeconds, 20); assert.equal(settings.pace.deleteDialogDelay.minSeconds, 5); assert.equal(settings.pace.deleteDialogDelay.maxSeconds, 25); });
test('默认启用持续运行并使用 10 至 60 分钟刷新休息', () => { const settings = context.SocialCommentPaceConfig.normalizeSettings({}); assert.equal(settings.sessionMaxMinutes, 0); assert.equal(settings.pace.refreshRest.minSeconds, 600); assert.equal(settings.pace.refreshRest.maxSeconds, 3600); assert.equal(settings.pace.refreshRest.meanSeconds, 1800); });
test('刷新休息配置拒绝超出 10 至 60 分钟的输入', () => { assert.throws(() => context.SocialCommentPaceConfig.validateSettings({ pace: { refreshRest: { minSeconds: 300, meanSeconds: 1800, maxSeconds: 3600 } } }), /10～60/); assert.throws(() => context.SocialCommentPaceConfig.validateSettings({ pace: { refreshRest: { minSeconds: 600, meanSeconds: 3600, maxSeconds: 3601 } } }), /10～60/); });
...
test('会话删除上限支持不限', () => { const settings = context.SocialCommentPaceConfig.validateSettings({ sessionLimit: 'unlimited' }); assert.equal(settings.sessionLimit, 'unlimited'); });
test('自动加载配置固定开启且边界不接受用户覆盖', () => {
  const settings = context.SocialCommentPaceConfig.normalizeSettings({ pagination: { enabled: false, maxBatches: 7, noGrowthAttempts: 2, stableWaitMs: 300, allowDeletion: false } });
  assert.equal(context.SocialCommentPaceConfig.normalizeSettings({}).pagination.enabled, true);
...
test('评论容器分帧滚动使用固定初始目标和可预测梯形曲线', async () => {
  ...
  assert.ok(context.frameCallCount > 0);
...
test('评论快照签名与对象遍历顺序无关', () => {
  const stability = context.SocialCommentSurfaceStability;
...
test('日文一级评论子级展开入口可被识别', () => {
  const labels = fs.readFileSync('src/platform/instagram/control-labels.js', 'utf8');
  assert.match(labels, /\^\\d\+件すべての返信を見る\$/);
  assert.match(labels, /\^返信をすべて見る\$/);
  assert.doesNotMatch(labels, /\^返信\$/);
});
test('回复展开确认只检查当前评论容器的加载状态', () => {
  const source = fs.readFileSync('src/platform/instagram/loader.js', 'utf8');
  assert.match(source, /const loadingRoot = beforeState\.container \|\| control\?\.parentElement \|\| surface \|\| documentRef/);
  assert.match(source, /stable: !findLoadingIndicator\(loadingRoot\)/);
});
test('多语言加载更多评论入口可被识别', () => {
  const labels = fs.readFileSync('src/platform/instagram/control-labels.js', 'utf8');
  ['加载更多|查看更多|查看全部', 'load|view|see', 'コメント|返信'].forEach((fragment) => assert.match(labels, new RegExp(fragment)));
  assert.doesNotMatch(labels, /loadMore: \[\s*\/\^回复\$\//);
});
test('运行时只使用 DOM，不安装接口响应观察器', () => {
  ...
  const panel = fs.readFileSync('src/content/cleaner-panel.js', 'utf8');
  const surface = fs.readFileSync('src/platform/instagram/surface.js', 'utf8');
  const comments = fs.readFileSync('src/platform/instagram/comments.js', 'utf8');
  const actions = fs.readFileSync('src/platform/instagram/actions.js', 'utf8');
  const loader = fs.readFileSync('src/platform/instagram/pagination-loader.js', 'utf8');
  ...
  assert.equal(runtimeScripts.includes('src/shared/comment-surface-stability.js'), true);
  assert.equal(runtimeScripts.includes('src/platform/instagram/pagination-surface.js'), true);
  assert.equal(runtimeScripts.includes('src/platform/instagram/pagination-controls.js'), true);
  assert.equal(runtimeScripts.includes('src/platform/instagram/pagination-loader.js'), true);
  assert.equal(runtimeScripts.includes('src/platform/instagram/reply-expansion.js'), true);
  assert.equal(runtimeScripts.includes('src/content/cleaner-panel.js'), true);
  assert.equal(runtimeScripts.includes('src/content/social-comment-cleaner.js'), false);
  assert.equal(/\bfetch\b|XMLHttpRequest/.test(panel + surface + comments + actions), false);
  assert.equal(/\bfetch\b|XMLHttpRequest/.test(loader), false);
  assert.equal(/ICC_HOVER_COMMENT|chrome\.debugger|Input\.dispatchMouseEvent/.test(panel + worker), false);
  assert.match(worker, /ICC_GET_SETTINGS/);
  assert.equal(/chrome\.storage\.sync\.get/.test(panel), false);
  assert.match(comments, /InstagramControlLocator/);
  assert.match(comments, /deriveReplyParentIds/);
  assert.equal(/closest\(['"]ul|isReply:\s*Boolean\(link\.closest\(['"]ul/.test(comments), false);
});
test('Preview 与正式模式共用一级评论串行编排，但 Preview 不进入删除分支', () => {
  const source = fs.readFileSync('src/core/cleaner-runtime.js', 'utf8');
  assert.equal(source.includes('processPreview'), false);
  assert.match(source, /this\.session\.mode !== 'preview' && this\.session\.candidates\.length/);
  assert.match(source, /if \(this\.session\.mode === 'preview'\) return this\.stop\('completed'/);
});
